...
export function AnnouncementModal({ open, onClose, onSaved, categories }: AnnouncementModalProps) {
  ...
  const handleSaveDraft = async () => {
    if (!title && !content && !fileUrl) return false;
    ...
    const noticeType = fileUrl ? (isImage ? "image" : isPdf ? "pdf" : "file") : "text";
    try {
      const res = await fetch("/api/notices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: title || "Untitled draft", description: content, type: noticeType, content,
  ...
  const { backdropProps, requestClose, confirmDialog } = useModalCloseGuard({ open,
    onClose, hasContent: () => title.trim() !== "" || content.trim() !== "" || !!fileUrl, onSaveDraft: handleSaveDraft,
  });
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/notices", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title, description: content, type: noticeType, content,
      ...
      <div className="bg-card border border-border/40 rounded-t-2xl rounded-b-none sm:rounded-2xl shadow-2xl w-full max-w-lg max-h-[95dvh] sm:max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/40 shrink-0">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl flex items-center justify-center text-white bg-orange-500">
              <Megaphone className="h-5 w-5" />
            </div>
            <h2 className="text-base font-bold">Announcement</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="space-y-1.5">
            <Label>Title</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Announcement title" className="rounded-xl" />
          </div>
          <div className="space-y-1.5">
            <Label>Content</Label>
            <Textarea value={content} onChange={(e) => setContent(e.target.value)} rows={5} placeholder="Type your announcement..." className="rounded-xl" />
          </div>
          <div className="space-y-1.5">
        ...
        <div className="px-5 py-3 border-t border-border/40 shrink-0 flex justify-end gap-2">
          ...
          <Button onClick={handleSave} disabled={!title || !content || saving} className="bg-indigo-600 hover:bg-indigo-700 rounded-xl">
